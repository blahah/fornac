...
export function rnaPlot () {
  ...
  function createTransformToFillViewport (xValues, yValues, molName = '') {
    ...
    function createOtherScale (firstScale, newDomain, newRange) {
      const scaleFactor = (firstScale.range()[1] - firstScale.range()[0]) /
    ...
    const xOffset = xScale.range()[0] - xScale.domain()[0]
    const yOffset = yScale.range()[0] - yScale.domain()[0]
  ...
  function createLabels (selection, labelNodes) {
    ...
    const numberLabels = gs.append('svg:text')
      .text(function (d) { return d.name })
      .attr('text-anchor', 'middle')
  ...
  function makeExternalLinksBundle (selection, links) {
    ...
    const fbundling = d3.ForceEdgeBundling().nodes(nodesDict).edges(linksList)
  ...
  function createLinks (selection, links) {
    ...
    const gs = selection.selectAll('.rna-link')
      .data(links)
      .enter()
      .append('svg:line')
  ...
  function chart (selection) {
    selection.each(function (data) {
      ...
      const rg = new RNAGraph(data.sequence, data.structure, data.name)
        .recalculateElements()
        .elementsToJson()
        .addName(data.name)
      ...
      const gTransform = d3.select(this)
        .append('g')
        .attr('transform', fillViewportTransform)

      const nucleotideNodes = rg.nodes.filter(function (d) {
        return d.nodeType == 'nucleotide'
      })

      const labelNodes = rg.nodes.filter(function (d) {
...
const number_sort = function (a, b) { return a - b }
...
function RNAUtilities () {
  ...
  self.mm_bt = function (mm, pt, old_pt, i, j) {
    ...
    for (let q = j - TURN - 1; q >= i; q--) { /* j is paired with some q */
      if (old_pt[j] !== q) { continue }

      const left_part = (q > i) ? mm[i][q - 1] : 0
      const enclosed_part = (j - q - 1 > 0) ? mm[q + 1][j - 1] : 0
  ...
  self.removePseudoknotsFromPairtable = function (pt) {
    ...
    const mm = self.maximumMatching(pt)
    const new_pt = self.backtrackMaximumMatching(mm, pt)
    const removed = []
