...
NAView.prototype.traverse_loop = function traverse_loop (lp, anchor_connection) {
  let xs, ys, xe, ye, xn, yn, angleinc, r
  let radius, xc, yc, xo, yo, astart, aend, a
  let cp, cpnext, acp, cpprev
  let i, j, n, ic
  let da, maxang
  let count, icstart, icend, icmiddle, icroot
  let done, done_all_connections, rooted
  let sign
  let midx, midy, nrx, nry, mx, my, vx, vy, dotmv, nmidx, nmidy
  let icstart1, icup, icdown, icnext, direction
  let dan, dx, dy, rr
  let cpx, cpy, cpnextx, cpnexty, cnx, cny, rcn, rc, lnx, lny, rl, ac, acn, sx, sy, dcp
  let imaxloop = 0
...
NAView.prototype.determine_radius = function determine_radius (lp, lencut) {
  let mindit, ci, dt, sumn, sumd, radius, dit
  let i; let j; let end; let start; let imindit = 0
  let cp = new Connection(); let cpnext = new Connection()
...
NAView.prototype.construct_extruded_segment = function construct_extruded_segment (cp, cpnext) {
  let astart, aend1, aend2, aave, dx, dy, a1, a2, ac, rr, da, dac
  let start, end, n, nstart, nend
  let collision
...
NAView.prototype.construct_circle_segment = function construct_circle_segment (start, end) {
  let dx, dy, rr, midx, midy, xn, yn, nrx, nry, mx, my, a
  let l, j, i
...
NAView.prototype.find_center_for_arc = function find_center_for_arc (n, b) {
  let h, hhi, hlow, r, disc, theta, e, phi
  let iter
