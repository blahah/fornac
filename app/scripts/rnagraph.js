...
const numberSort = function (a, b) { return a - b }
...
export function RNAGraph (seq, dotbracket, structName, startNumber) {
  ...
  self.addUids = function (uids) {
    const nucleotideNodes = self.nodes.filter(function (d) { return d.nodeType == 'nucleotide' })

    for (let i = 0; i < uids.length && i < nucleotideNodes.length; i++) { nucleotideNodes[i].uid = uids[i] }
  ...
  self.addPositions = function (nodeType, positions) {
    const labelNodes = self.nodes.filter(function (d) { return d.nodeType == nodeType })
  ...
  self.breakNodesToFakeNodes = function () {
    ...
    const labelNodes = self.nodes.filter(function (d) { return d.nodeType == 'nucleotide' })
  ...
  self.getPositions = function (nodeType) {
    const positions = []
    const nucleotideNodes = self.nodes.filter(function (d) { return d.nodeType == nodeType })

    for (let i = 0; i < nucleotideNodes.length; i++) { positions.push([nucleotideNodes[i].x, nucleotideNodes[i].y]) }
  ...
  self.reinforceStems = function () {
    ...
    const relevantElements = self.elements.filter(function (d) {
  ...
  self.connectFakeNodes = function () {
    ...
    const filterOutNonFakeNodes = function (d) {
  ...
  self.addLabels = function (startNumber, labelInterval) {
    ...
    for (let i = 1; i <= self.pairtable[0]; i++) {
      // add labels
      if (i % labelInterval === 0) {
        ...
        const vecLength = Math.sqrt(combinedVec[0] * combinedVec[0] + combinedVec[1] * combinedVec[1])
...
export function moleculesToJson (moleculesJson) {
  ...
  const nodes = {} // index the nodes by uid
  const graphs = []
  const extraLinks = []
